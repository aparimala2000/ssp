...
<div class="d-flex justify-content-between align-items-center mb-30">
	...
	<span class="ld">
			...
			<ul id="shipping_method">
				<?php foreach ($available_methods as $method) : ?>
					<li>
						<?php
						printf(
							'<input type="radio" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d_%2$s" value="%3$s" class="shipping_method free_ship" %4$s />
		...
		<?php elseif (1 === count($available_methods)) :  ?>
			...
			printf('%3$s <input type="hidden" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d" value="%2$s" class="shipping_method ship_cnt" />', $index, esc_attr($method->id), wc_price(WC()->cart->shipping_total));
			do_action('woocommerce_after_shipping_rate', $method, $index);
			?>
